...
void CRenderTarget::phase_hud_intoxication()
{
    u32 Offset = 0;
    Fvector2 p0, p1;

    float ActorCurrentIntoxication = g_pGamePersistent->GetActorIntoxication();
    ...
    // Draw COLOR
    RCache.set_Element(s_hud_intoxication->E[0]);
    RCache.set_c("hud_intoxication_params", ActorCurrentIntoxication, 0, 0, 0);
    RCache.set_Geometry(g_aa_AA);
    RCache.Render(D3DPT_TRIANGLELIST, Offset, 0, 4, 0, 2);
