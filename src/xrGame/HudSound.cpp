...
void HUD_SOUND_ITEM::LoadSound(LPCSTR section, LPCSTR line, HUD_SOUND_ITEM& hud_snd, int type)
{
    ...
    while (pSettings->line_exist(section, sound_line))
    {
        ...
        LoadSound(section, sound_line, s.snd, type, &s.volume, &s.freq, &s.delay);
        xr_sprintf(sound_line, "%s%d", line, ++k);
    } // while
}

void HUD_SOUND_ITEM::LoadSound(LPCSTR section, LPCSTR line, ref_sound& snd, int type, float* volume, float* freq, float* delay)
{
...
void HUD_SOUND_ITEM::PlaySoundAdd(HUD_SOUND_ITEM& hud_snd, const Fvector& position, const IGameObject* parent, bool b_hud_mode, bool looped, u8 index)
{
	...
	hud_snd.m_activeSnd->snd.play_no_feedback(const_cast<IGameObject*>(parent), flags, hud_snd.m_activeSnd->delay, &pos, &vol, &g_fHudSndFrequency);
}
