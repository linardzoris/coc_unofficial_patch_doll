...
void CPseudoGigant::on_threaten_execute()
{
    ...
    for (u32 i = 0; i < m_nearest.size(); i++)
    {
        ...
        if (!obj || !obj->m_pPhysicsShell || (obj->spawn_ini() && obj->spawn_ini()->section_exist("ph_heavy")) || (pSettings->line_exist(obj->cNameSect().c_str(), "ph_heavy") && pSettings->r_bool(obj->cNameSect().c_str(), "ph_heavy")) ||
