...
void CCustomZone::Load(LPCSTR section)
{
	...
	m_bDestroyAfterBlowout = READ_IF_EXISTS(pSettings, r_bool, section, "DestroyAfterBlowout", false);
            ...
            if (grassbender_id)
            {
                ...
                if (GData.id[grassbender_id] == ID())
                {
                    GData.str_target[grassbender_id] +=
                        g_pGamePersistent->GrassBenderToValue(GData.str_target[grassbender_id], 0.0f, 4.0f, false);
