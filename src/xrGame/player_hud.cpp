...
void player_hud::load(const shared_str& player_hud_sect)
{
    ...
    m_model_2 = smart_cast<IKinematicsAnimated*>(GEnv.Render->model_Create(pSettings->line_exist(player_hud_sect, "visual_2") ? pSettings->r_string(player_hud_sect, "visual_2") : model_name.c_str()));
...
void player_hud::detach_item_idx(u16 idx)
{
    ...
    else if (idx == 0)
    {
        if (m_attached_items[1])
        {
            // fix for a rare case where the right hand stays visible on screen after detaching the right hand's
            // attached item
